...
2. If an item's quantity is missing, or its preparation method is missing AND materially changes the calorie estimate (for example: fried vs. boiled vs. raw), add an entry to "clarifications_needed" asking the user, rather than assuming.
3. List any assumptions you made (e.g. assumed cooking method, assumed portion size) in "assumptions".
4. Respond with JSON only, matching the provided schema exactly.`;
