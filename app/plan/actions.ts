...
export async function generatePlanAction(): Promise<PlanGenerationState> {
  return withActionLogging("generatePlanAction", async () => {
...
export async function approvePlanAction(
  ...
  return withActionLogging("approvePlanAction", async () => {
    ...
    try {
      ...
      logger.error({ route: "approvePlanAction", error: String(err) }, "db write failed");
      return {
        ok: false,
